...
import numpy as np
import cv2
from det2d.util import distance2bbox, bbox2distance, overlay_bbox_cv, multi_apply, images_to_levels
...
from det2d.util import warp_boxes
...
class GFLHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.loss_dfl = DistributionFocalLoss(loss_weight=self.loss_cfg['loss_qfl']['loss_weight'])
    ...
    def target_assign_single_img(self,
        ...
        if 0 in gt_bboxes.shape:
            gt_bboxes = torch.zeros((0, 4), device=device, dtype=torch.float32)
        else:
